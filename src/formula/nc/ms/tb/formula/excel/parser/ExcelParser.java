// $ANTLR : "Excel.g" -> "ExcelParser.java"$
...
public class ExcelParser extends antlr.LLkParser       implements ExcelParserTokenTypes
 {

protected ExcelParser(TokenBuffer tokenBuf, int k) {
  super(tokenBuf,k);
  tokenNames = _tokenNames;
}

public ExcelParser(TokenBuffer tokenBuf) {
  this(tokenBuf,1);
}

protected ExcelParser(TokenStream lexer, int k) {
  super(lexer,k);
  tokenNames = _tokenNames;
}

public ExcelParser(TokenStream lexer) {
  this(lexer,1);
}

public ExcelParser(ParserSharedInputState state) {
  super(state,1);
  tokenNames = _tokenNames;
}
	...
	public final FunctionCall  function() throws RecognitionException, TokenStreamException {
		...
		try {      // for error handling
			...
			case FUNCTION: // '\030'
							for (; LA(1) == COMMA;) {
								match(COMMA);
								Literal zeroNode = new Literal();
								zeroNode.setStatement("0");
								fc.addArgument(zeroNode);
							}
